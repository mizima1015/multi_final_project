...
def add_bg_from_url():
    max_val = max(st.session_state.predictions) if st.session_state.predictions else '데이터 없음'
    min_val = min(st.session_state.predictions) if st.session_state.predictions else '데이터 없음'
    avg_prediction = np.mean(st.session_state.predictions) if st.session_state.predictions else '데이터 없음'

    st.markdown(
         f"""
         <style>
         .stApp {{
             background-image: url("https://i.imgur.com/zB1cUBc.jpg");
             background-attachment: fixed;
             background-size: cover; /* 이미지가 컨테이너를 가득 채우도록 */
             background-position: center; /* 이미지를 컨테이너의 중앙에 위치시킴 */
            ...
            예측 된 물류량 최댓값 = {max_val} \n
            예측 된 물류량 최솟값 = {min_val} \n
            최댓값 최솟값 차이 = ㅌㅌㅌ \n
